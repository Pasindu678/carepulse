...
export default function DashboardView({ onBackToSetup, patient, setPatient, device, setDevice }) {
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row min-h-[calc(100vh-65px)] bg-[#F8FAFC] dark:bg-[#0B0F19] transition-colors">
      ...
      <main className="flex-1 p-4 sm:p-8 max-w-[1240px] overflow-y-auto space-y-6">
        ...
        {/* Top Breadcrumb & User Greeting */}
        <div>
          <button
            onClick={onBackToSetup}
            className="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1 mb-2 transition-colors cursor-pointer"
          >
      ...
      {activeContactModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-100 dark:border-slate-800 text-center relative">
            <button
            ...
            {activeContactModal === 'call' && (
              <div className="space-y-4">
                <div className="w-16 h-16 rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mx-auto animate-bounce">
                  <Phone className="w-8 h-8 fill-emerald-600 dark:fill-emerald-400" />
                </div>
                <h4 className="text-lg font-bold text-slate-900 dark:text-white">Calling Wristband...</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Connecting high-definition two-way audio to {patient?.name || "Mrs. Nirmala Perera"}'s CarePulse Band Pro.
                </p>
                <button
                  onClick={() => setActiveContactModal(null)}
                  className="w-full py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold cursor-pointer"
                >
                  End Call
                </button>
              </div>
            ...
            {activeContactModal === 'video' && (
              <div className="space-y-4">
                <div className="w-16 h-16 rounded-full bg-blue-100 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 flex items-center justify-center mx-auto">
                  <Video className="w-8 h-8" />
                </div>
                <h4 className="text-lg font-bold text-slate-900 dark:text-white">Live Video Check-in</h4>
                <div className="h-32 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center text-white text-xs">
                  Connecting to Home Camera Hub...
                </div>
                <button
                  onClick={() => setActiveContactModal(null)}
                  className="w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold cursor-pointer"
                >
                  Close Video
                </button>
              </div>
            ...
            {activeContactModal === 'message' && (
              <div className="space-y-3 text-left">
                <h4 className="text-base font-bold text-slate-900 dark:text-white">Send Wristband TTS Audio Message</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-xs focus:ring-2 focus:ring-purple-500/20 focus:outline-none"
                />
                <button
                  onClick={() => {
                  ...
                  className="w-full py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold cursor-pointer"
                >
                  Speak On Wristband
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 dark:border-slate-800 relative">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h4 className="font-bold text-slate-900 dark:text-white text-base">{selectedAlert.title}</h4>
                <p className="text-xs text-slate-400 dark:text-slate-500">{selectedAlert.timestamp}</p>
              </div>
            </div>
            <p className="text-xs text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/80 my-4 leading-relaxed">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2.5 bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-xl text-xs font-semibold cursor-pointer"
              >
                Call Now
              </button>
            </div>
      ...
      {mapModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 dark:bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-100 dark:border-slate-800 relative">
            ...
            <h4 className="font-bold text-slate-900 dark:text-white text-lg mb-1">Live GPS Location</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">Gampaha, Western Province, Sri Lanka • Geofence Status: Safe Zone (Home)</p>
            <div className="h-72 rounded-2xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 map-grid-pattern relative flex items-center justify-center overflow-hidden">
              <div className="text-center">
                ...
                <div className="mt-2 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm px-4 py-2 rounded-xl text-xs font-bold text-slate-900 dark:text-white shadow-md border border-slate-200/60 dark:border-slate-700">
                  {patient?.name || "Mrs. Nirmala Perera"} • Home
                </div>
                <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Accuracy: ±3 meters • Satellite Lock: 9 Sats</div>
              </div>
            </div>
          </div>
        </div>
